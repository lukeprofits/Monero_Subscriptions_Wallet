...
class SubscriptionsUI(RecycleView):
    ...
    def load_subscriptions_data(self):
        return [{'amount': str(sub.amount), 'custom_label': sub.custom_label, 'renewal_date': sub.renewal_date(), 'billing_cycle_days': sub.billing_cycle_days} for sub in self.subscriptions.read_subscriptions()]
